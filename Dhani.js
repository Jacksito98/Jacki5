...
case 'ttp':
	if (!isGroup) return reply(mess.only.group)	
if (args.length == 0) return reply('▢ Ejemplo de uso : /attp Hola')
...
case 'tomp4':
	case 'tovid':
		case 'tovideo':
			if (!isGroup) return reply(mess.only.group)	
				if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
				ger = isQuotedSticker ? JSON.parse(JSON.stringify(choute).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
...
case 'sider':
if (!isGroup) return reply(mess.only.group)
infom = await Dhani.messageInfo(from, choute.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
...
case 'autodemote':
                if (isBan) return reply (mess.only.group)	
                if (!isGroup) return reply(mess.only.group)
                if (!isAdmin) return reply(mess.only.group)
                if (!botAdmin) return reply(mess.only.group)
                if (botAdmin) reply(`Tabien`)                                                                     
Dhani.groupDemoteAdmin(from, [sender])
setTimeout( () => {
Dhani.groupDemoteAdmin(from, [sender]) }, 2000) 
break
		...
		case 'yo':
			case 'miperfil':
			  try {
				...
				tu = await Dhani.getStatus(`${sender.split('@')[0]}@c.us`, MessageType.text)
