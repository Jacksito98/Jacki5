...
//━━━━[ Bot ]━━━━//
		...
		case 'block':
				...
				Dhani.blockUser (`${body.slice(8)}@c.us`, "add")
				Dhani.sendMessage(from, `Usuario bloqueado`, text, {
			  quoted: mek
				})
				break	
		...
		case 'unblock':
				if (!isGroup) return reply(mess.only.group)
				if (!isOwner) return reply(mess.only.ownerB)
				Dhani.blockUser (`${body.slice(9)}@c.us`, "remove")
				Dhani.sendMessage(from, `Usuario desbloqueado`, text)
				break
...
Dhani.sendMessage(from, yoi, text, {
  quoted: mek
})
